...
public final class MohistKite {
    private static JsonObject versionJson;
    private static MohistKitePluginLoader loader;
    ...
    @SuppressWarnings("unused")
    @NotNull
    public static MohistKitePluginLoader getPluginLoader() {
        return loader;
    }

    @SuppressWarnings("ProtectedMemberInFinalClass")
    protected static void init() throws Throwable {
        try (InputStream is = MohistKiteAgent.getResourceAsStream("version.json")) {
            if (is != null) versionJson = new JsonParser().parse(new InputStreamReader(is)).getAsJsonObject();
        }
        Path pluginsPath = Paths.get("MohistKite/plugins");
        Files.createDirectories(pluginsPath);
        loader = new MohistKitePluginLoader();

        loadPlugins(pluginsPath).forEach(it -> {
            try {
                MohistKiteAgent.LOGGER.info("Loading " + it.getDescription().getFullName());
                it.onLoad();
            } catch (Throwable ex) {
                ex.printStackTrace();
                MohistKiteAgent.LOGGER.log(Level.SEVERE, ex.getMessage() + " initializing " +
        ...
        for (Iterator<Path> iter = Files.list(pluginsPath)
            ...
            try {
                ...
                if (name.equalsIgnoreCase("bukkit") || name.equalsIgnoreCase("minecraft") || name.equalsIgnoreCase("mojang")) {
                    MohistKiteAgent.LOGGER.log(Level.SEVERE, "Could not load '" + file.getPath() + "': Restricted Name");
                    continue;
                } else if (description.getRawName().indexOf(' ') != -1) {
                    MohistKiteAgent.LOGGER.log(Level.SEVERE, "Could not load '" + file.getPath() + "': uses the space-character (0x20) in its name");
                    continue;
                }
            } catch (InvalidDescriptionException ex) {
                ex.printStackTrace();
                MohistKiteAgent.LOGGER.log(Level.SEVERE, "Could not load '" + file.getPath() + "'", ex);
                continue;
            }

            File replacedFile = plugins.put(description.getName(), file);
            if (replacedFile != null) {
                MohistKiteAgent.LOGGER.severe(String.format("Ambiguous plugin name `%s' for files `%s' and `%s'",
            ...
            if (removedProvided != null) {
                MohistKiteAgent.LOGGER.warning(String.format("Ambiguous plugin name `%s'. It is also provided by `%s'",
            ...
            for (String provided : description.getProvides()) {
                ...
                if (pluginFile != null) {
                    MohistKiteAgent.LOGGER.warning(String.format("`%s provides `%s' while this is also the name of `%s'",
                            file.getPath(), provided, pluginFile.getPath()));
                } else {
                    String replacedPlugin = pluginsProvided.put(provided, description.getName());
                    if (replacedPlugin != null) {
                        MohistKiteAgent.LOGGER.warning(String.format("`%s' is provided by both `%s' and `%s'",
            ...
            if (missingDependency) {
                ...
                if (missingDependency) {
                    ...
                    while (failedPluginIterator.hasNext()) {
                        ...
                        MohistKiteAgent.LOGGER.log(Level.SEVERE, "Could not load '" + file.getPath() + "': circular dependency detected");
                    }
                }
            }
        }
